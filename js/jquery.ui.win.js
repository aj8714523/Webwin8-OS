	...
	$.extend({
		...
		arrayDiskIcon:function(width, height){	//自动排列桌面图标
			...
			for(var i=0,len=icons.length; i<len; i++){
				icons.eq(i).css({'left':parseInt(i/n)*width+15+'px','top':i%n*height+15+'px'});
				// .click(function(){	//单击选中当前图标
			...
			$(".diskicon ul").on("dblclick",'li',function(){
				//单击选中当前图标
				$(this).addClass('active').parent().find('li').not(this).removeClass('active');
			});
			$(".diskicon ul").on("dblclick",'li',function(){
		...
		openWin:function(settings, options){		//打开新窗口
			...
			//创建窗体对象
			var win = $('<div class="panel window" appid="' + appid + '" id="' + appid + '" status="'+ propertyString.winstate +'" path="' + settings.attr('path') +'"></div>');
			...
			win.fadeTo(500, 1);
//#############################修改
			var taskbar = $('<li class="app" appid="' + appid + '" path="' + settings.attr('path')+'"><span class="taskbar-body-icon ' + propertyString.icon + '"></span><span class="taskbar-body-text">' + propertyString.title + '</span><a href="javascript:tes(\''+appid+'\');"><span class="taskbar-close">x</span></a></li>').click(function(){
				$.taskbarClick($(this));
			});
			...
			if(propertyString.target == 'iframe'){
				winContent.append('<iframe  scrolling="yes" src="' + settings.attr('path') + '" frameborder="0" height="100%" width="100%" id="dialogFrame"></iframe>');				
			}
